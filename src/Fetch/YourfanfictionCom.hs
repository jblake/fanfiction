...
module Fetch.YourfanfictionCom
  ( peek
  , fetch
  , parse
  )
...
fetchChapter n infoUnique infoStoryID = do
  ...
  (uri, resp) <- request $ mkRequest GET $ fromJust $ parseURI $ "http://yourfanfiction.com/viewstory.php?action=printable&ageconsent=ok&warning=5&sid=" ++ urlEncode infoStoryID ++ "&chapter=" ++ urlEncode (show n)
    ...
    (2, 0, 0) -> do
      ...
      let
        ...
        (titleText, authorText) = case snd $ getTag (~== TagOpen ("div" :: T.Text) [("id", "pagetitle")]) page of
          ...
          _ -> error $ "Can't parse title/author in yourfanfiction.com/" ++ infoStoryID ++ "/" ++ show n

        chpTitleText = case snd $ getTag (~== TagOpen ("div" :: T.Text) [("class", "chaptertitle")]) page of
          [TagText t] -> t
          _ -> error $ "Can't find chapter title in yourfanfiction.com/" ++ infoStoryID ++ "/" ++ show n
        ...
        postedRegex = makeRegex ("Published: ([0-9]{1,2}) ([a-zA-Z]{3}) ([0-9]{4})" :: String) :: Regex
        updatedRegex = makeRegex ("Updated: ([0-9]{1,2}) ([a-zA-Z]{3}) ([0-9]{4})" :: String) :: Regex

        infoUpdated = case updatedRegex `match` infoString of
          [[_,d,m,y]] -> UTCTime (fromGregorian (read y) (monthNumber m) (read d)) 0
          _ -> case postedRegex `match` infoString of
            [[_,d,m,y]] -> UTCTime (fromGregorian (read y) (monthNumber m) (read d)) 0
            _ -> error $ "Can't parse date in yourfanfiction.com/" ++ infoStoryID ++ "/" ++ show n
